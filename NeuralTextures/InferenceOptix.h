...
bool SampleLatentGrid(
    cudaTextureObject_t latentTexture,
    float2 uv,
    float neuralLod,
    ...
    vecT& outputArray )
{
    ...
    uv.x -= dx * 0.5f; // Move samples from pixel centers to pixel corners
    uv.y -= dy * 0.5f;
    ...
    T_VEC_OUT&      outputArray )
{
    ...
    T_VEC_MAT_OUT z_i1 =
        ...
        >(
            ...
            (CUdeviceptr)weights,          // matrix
            weightsOffsetInBytes,
            (CUdeviceptr)weights,          // bias array
            biasOffsetInBytes
        );
...
bool SampleTextureSet( T_VEC_OUTPUT& outputLayer, NtcTextureSetConstants& tsc, 
    cudaTextureObject_t latentTexture, int numLatentFeatures, int latentWidth, int latentHeight,
    uint8_t* mlpWeights, int x, int y, int mipLevel )
{
