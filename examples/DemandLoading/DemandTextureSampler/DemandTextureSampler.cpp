...
void stdoutLogCallback( int level, const char* message )
{
    (void)level;
    printf( "[DL] %s\n", message );
}
...
const char* textureFunctionNames[] = {
    "tex2D",
    "tex2DLod",
    "tex2DGrad",
    "tex2DGradUdim",
...
struct OptixState
{
    ...
    int                         numTextureSamples     = 0;   // Number of texture samples to launch
    TextureSample textureSamples[MAX_TEXTURE_SAMPLES] = {};  // Host-side copy of texture samples
...
class DemandTextureSampler
{
  ...
  protected:
    demandLoading::Options m_demandLoadOptions{};
    demandLoading::TextureDescriptor m_textureDesc{};
    std::map<std::string, int> m_textureNameToId{};
    std::map<int, std::string> m_textureIdToName{};
...
bool DemandTextureSampler::processCommand( const std::string& line )
{
    ...
    {
        m_demandLoadOptions.maxRequestedPages = std::stoi( tokens[1] );
        return true;
    }
    else if( command == "useSparseTextures" )
    {
        m_demandLoadOptions.useSparseTextures = !(tokens[1][0] == 'f' || tokens[1][0] == 'F');
        return true;
    }
    else if( command == "useCascadingTextureSizes" )
    {
        m_demandLoadOptions.useCascadingTextureSizes = !(tokens[1][0] == 'f' || tokens[1][0] == 'F');
        return true;
    }
    else if( command == "coalesceWhiteBlackTiles" )
    {
        m_demandLoadOptions.coalesceWhiteBlackTiles = !(tokens[1][0] == 'f' || tokens[1][0] == 'F');
        return true;
    }
    else if( command == "coalesceDuplicateImages" )
    {
        m_demandLoadOptions.coalesceDuplicateImages = !(tokens[1][0] == 'f' || tokens[1][0] == 'F');
        return true;
    }
    ...
    else if( command == "filterMode" )
    {
        ...
        if( tokens[1] == "linear" ) m_textureDesc.filterMode = FILTER_BILINEAR;
    ...
    {
        m_optixState.numTextureSamples = 0;
        return true;
    }
    else if( command == "tex2D" )
    {
        ...
        m_optixState.textureSamples[m_optixState.numTextureSamples++] = sample;
        return true;
    }
...
{
    OTK_ERROR_CHECK( cuMemFree( m_optixState.d_textureSamples ) );
    OTK_ERROR_CHECK( cuMemFree( m_optixState.d_params ) );
}
...
bool DemandTextureSampler::init(  )
{
    ...
    // Set demand load logger
    DemandLoadLogger::setLogFunction( stdoutLogCallback, 100 );
    ...
    // Initialize OptiX context
    {
        ...
        otk::util::setLogger( optixOptions );
        OTK_ERROR_CHECK( optixDeviceContextCreate( cuCtx, &optixOptions, &m_optixState.optixContext ) );
    }

    // Initialize DemandLoader
    m_optixState.demandLoader.reset( createDemandLoader( m_demandLoadOptions ), demandLoading::destroyDemandLoader );
    ...
    // Create OptiX module
    {
        ...
        OTK_ERROR_CHECK_LOG( optixModuleCreate( context, &module_compile_options, &pipeline_compile_options,
            DemandTextureSamplerKernelCudaText(), DemandTextureSamplerKernelCudaSize, LOG, &LOG_SIZE, &module ) );
    }
...
bool DemandTextureSampler::loadTexture( std::vector<std::string>& tokens )
{
...
#ifdef INCLUDE_NTC_TEXTURES
    else if( textureName.size() >= 4 && textureName.substr( textureName.size() - 4 ) == ".ntc" )
    {
    ...
    m_textureNameToId[textureName] = texture.getId();
    m_textureIdToName[texture.getId()] = textureName;

    return true;
}

int DemandTextureSampler::getTextureId( const std::string& textureName )
{
    auto it = m_textureNameToId.find( textureName );
...
bool DemandTextureSampler::launch()
{
    init();
    ...
    // Pull the samples back from the device and print them
    TextureSample rsamples[MAX_TEXTURE_SAMPLES];
    OTK_ERROR_CHECK( cuMemcpy( rsamples, m_optixState.d_textureSamples, sizeof( rsamples ), cudaMemcpyDeviceToHost ) );
    for( int i = 0; i < m_optixState.numTextureSamples; i++ )
    {
        printSample( rsamples[i] );
    }

    // Process demand load requests
...
void DemandTextureSampler::printSample( TextureSample& s )
{
    ...
    const char* funcName = textureFunctionNames[tfunc];
    ...
    if( tfunc == TF_tex2D )
    {
        printf("%s(%s, %0.4f, %0.4f) = ", funcName, m_textureIdToName[s.textureId].c_str(), s.u, s.v);
    }
    else if( tfunc == TF_tex2DLod )
    {
        printf("%s(%s, %0.4f, %0.4f, %0.4f) = ", funcName, m_textureIdToName[s.textureId].c_str(), s.u, s.v, s.lod);
    }
             ...
             || tfunc == TF_ntcTex2DGrad || tfunc == TF_ntcTex2DGradUdim )
    {
        printf("%s(%s, %0.4f, %0.4f, {%0.4f, %0.4f}, {%0.4f, %0.4f}) = ", 
            funcName, m_textureIdToName[s.textureId].c_str(), s.u, s.v, s.ddx.x, s.ddx.y, s.ddy.x, s.ddy.y);
    }
...
{
    (void)argv;
    DemandTextureSampler dtSampler;
    ...
    {
        if( !dtSampler.processCommand( line ) )
            break;
    }
    dtSampler.shutdown();
